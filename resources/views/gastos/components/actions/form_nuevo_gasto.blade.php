...
<div class="row g-1 form-group">
    ...
    @if($gasto->moc_imagen)
    <a href="{{ asset($gasto->moc_imagen) }}" target="_blank" class="btn btn-sm btn-info text-white choose-button">Descargar Recibo</a>
@endif
...
<div class="row g-1">
    <div class="col-md-6">
        ...
        <div class="input-group"><span class="input-group-text"><i class="fa fa-euro"></i></span>
            <input class="form-control" type="number" name="moc_monto" id="moc_monto" value="{{ $gasto->moc_monto ?? 0 }}"  placeholder="">
          </div>
    </div>
    <div class="col-md-6">
        <label class="form-label" for="moc_fecha">Fecha</label>
            ...
            <input class="form-control form-control-sm  datepicker-here" name="moc_fecha"
                id="moc_fecha" data-language="es" data-date-format="dd/mm/yyyy" type="text" value="{{ $gasto->moc_fecha ?? now() }}"  required="">
    ...
    <div class="col-md-6">
        ...
        <select class="form-select form-select-sm" name="forma_pago_id" id="forma_pago_id" required="">
            ...
            @foreach ($opcionesPagos as $opcionPago)
                <option value="{{ $opcionPago->id }}" {{ $gasto->forma_pago_id == $opcionPago['id'] ? 'selected=""' : '' }}>{{ $opcionPago->fop_nombre }}
                </option>
            @endforeach
        </select>
    ...
    <div class="col-md-6">
        ...
        <select class="form-select form-select-sm" name="categoria_id" id="categoria_id"
            ...
            @foreach ($categorias as $categoria)
                <option value="{{ $categoria->id }}" {{ $gasto->categoria_id == $categoria['id'] ? 'selected=""' : '' }}>{{ $categoria->cat_nombre }}</option>
            @endforeach
        </select>
    </div>
    <div class="col-md-6">
       ...
       <select class="form-select form-select-sm" name="sub_categoria_id" id="sub_categoria_id">
                ...
                {{-- @foreach($mayoristas as  $mayorista)
                        <option value="{{ $mayorista->id }}" {{ $gasto->sub_categoria_id == $mayorista['id'] ? 'selected=""' : '' }} >{{ $mayorista->may_nombre}}</option>
                    @endforeach --}}
            </select>
    </div>
</div>
<div class="row g-2">    
    <div class="col-md-12">
        <textarea class="form-control" name="moc_descripcion" id="moc_descripcion" placeholder="Descripcion" >{{ $gasto->moc_descripcion }}</textarea>
    </div>   
</div>
